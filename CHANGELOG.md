# Changelog

## Version History

* [v1.4.3](#v1.4.3)
* [v1.4.2](#v1.4.2)
* [v1.4.1](#v1.4.1)
* [v1.4.0](#v1.4.0)
* [v1.3.1](#v1.3.1)
* [v1.3.0](#v1.3.0)
* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.4.3"></a>
### [v1.4.3](/compare/v1.4.2...v1.4.3)

> 2026-06-05

#### 🚀 Enhancements

* **release:** include release tag in dry run output


<a name="v1.4.2"></a>
### [v1.4.2](/compare/v1.4.1...v1.4.2)

> 2026-06-05

#### 🏡 Chore

* **release:** prepare v1.4.2

#### 🚀 Enhancements

* **release:** add dry run output for version preview before release


<a name="v1.4.1"></a>
### [v1.4.1](/compare/v1.4.0...v1.4.1)

> 2026-06-05

#### 🏡 Chore

* **release:** prepare v1.4.1

#### 🚀 Enhancements

* **release:** add NODE_AUTH_TOKEN for npm publishing and update publishConfig


<a name="v1.4.0"></a>
### [v1.4.0](/compare/v1.3.1...v1.4.0)

> 2026-06-05

#### 🩹 Fixes

* **cli:** improve error handling for missing binary and streamline install script execution
* **release:** remove provenance and access flags from npm publish command

#### 🏡 Chore

* **release:** prepare v1.4.0

#### 🚀 Enhancements

* add npm support and installation script; update README and .gitignore


<a name="v1.3.1"></a>
### [v1.3.1](/compare/v1.3.0...v1.3.1)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** resolve bundled template path

#### 🏡 Chore

* **release:** prepare v1.3.1


<a name="v1.3.0"></a>
### [v1.3.0](/compare/v1.2.0...v1.3.0)

> 2026-05-05

#### 🏡 Chore

* **release:** prepare v1.3.0

#### 🚀 Enhancements

* **changelog:** add full history mode


<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 🏡 Chore

* **release:** prepare v1.2.0

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.4.2](#v1.4.2)
* [v1.4.1](#v1.4.1)
* [v1.4.0](#v1.4.0)
* [v1.3.1](#v1.3.1)
* [v1.3.0](#v1.3.0)
* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.4.2"></a>
### [v1.4.2](/compare/v1.4.1...v1.4.2)

> 2026-06-05

#### 🚀 Enhancements

* **release:** add dry run output for version preview before release


<a name="v1.4.1"></a>
### [v1.4.1](/compare/v1.4.0...v1.4.1)

> 2026-06-05

#### 🏡 Chore

* **release:** prepare v1.4.1

#### 🚀 Enhancements

* **release:** add NODE_AUTH_TOKEN for npm publishing and update publishConfig


<a name="v1.4.0"></a>
### [v1.4.0](/compare/v1.3.1...v1.4.0)

> 2026-06-05

#### 🩹 Fixes

* **cli:** improve error handling for missing binary and streamline install script execution
* **release:** remove provenance and access flags from npm publish command

#### 🏡 Chore

* **release:** prepare v1.4.0

#### 🚀 Enhancements

* add npm support and installation script; update README and .gitignore


<a name="v1.3.1"></a>
### [v1.3.1](/compare/v1.3.0...v1.3.1)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** resolve bundled template path

#### 🏡 Chore

* **release:** prepare v1.3.1


<a name="v1.3.0"></a>
### [v1.3.0](/compare/v1.2.0...v1.3.0)

> 2026-05-05

#### 🏡 Chore

* **release:** prepare v1.3.0

#### 🚀 Enhancements

* **changelog:** add full history mode


<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 🏡 Chore

* **release:** prepare v1.2.0

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.4.1](#v1.4.1)
* [v1.4.0](#v1.4.0)
* [v1.3.1](#v1.3.1)
* [v1.3.0](#v1.3.0)
* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.4.1"></a>
### [v1.4.1](/compare/v1.4.0...v1.4.1)

> 2026-06-05

#### 🚀 Enhancements

* **release:** add NODE_AUTH_TOKEN for npm publishing and update publishConfig


<a name="v1.4.0"></a>
### [v1.4.0](/compare/v1.3.1...v1.4.0)

> 2026-06-05

#### 🩹 Fixes

* **cli:** improve error handling for missing binary and streamline install script execution
* **release:** remove provenance and access flags from npm publish command

#### 🏡 Chore

* **release:** prepare v1.4.0

#### 🚀 Enhancements

* add npm support and installation script; update README and .gitignore


<a name="v1.3.1"></a>
### [v1.3.1](/compare/v1.3.0...v1.3.1)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** resolve bundled template path

#### 🏡 Chore

* **release:** prepare v1.3.1


<a name="v1.3.0"></a>
### [v1.3.0](/compare/v1.2.0...v1.3.0)

> 2026-05-05

#### 🏡 Chore

* **release:** prepare v1.3.0

#### 🚀 Enhancements

* **changelog:** add full history mode


<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 🏡 Chore

* **release:** prepare v1.2.0

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.4.0](#v1.4.0)
* [v1.3.1](#v1.3.1)
* [v1.3.0](#v1.3.0)
* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.4.0"></a>
### [v1.4.0](/compare/v1.3.1...v1.4.0)

> 2026-06-05

#### 🩹 Fixes

* **cli:** improve error handling for missing binary and streamline install script execution
* **release:** remove provenance and access flags from npm publish command

#### 🚀 Enhancements

* add npm support and installation script; update README and .gitignore


<a name="v1.3.1"></a>
### [v1.3.1](/compare/v1.3.0...v1.3.1)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** resolve bundled template path

#### 🏡 Chore

* **release:** prepare v1.3.1


<a name="v1.3.0"></a>
### [v1.3.0](/compare/v1.2.0...v1.3.0)

> 2026-05-05

#### 🏡 Chore

* **release:** prepare v1.3.0

#### 🚀 Enhancements

* **changelog:** add full history mode


<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 🏡 Chore

* **release:** prepare v1.2.0

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.3.1](#v1.3.1)
* [v1.3.0](#v1.3.0)
* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.3.1"></a>
### [v1.3.1](/compare/v1.3.0...v1.3.1)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** resolve bundled template path


<a name="v1.3.0"></a>
### [v1.3.0](/compare/v1.2.0...v1.3.0)

> 2026-05-05

#### 🏡 Chore

* **release:** prepare v1.3.0

#### 🚀 Enhancements

* **changelog:** add full history mode


<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 🏡 Chore

* **release:** prepare v1.2.0

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.3.0](#v1.3.0)
* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.3.0"></a>
### [v1.3.0](/compare/v1.2.0...v1.3.0)

> 2026-05-05

#### 🚀 Enhancements

* **changelog:** add full history mode


<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 🏡 Chore

* **release:** prepare v1.2.0

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.2.0](#v1.2.0)
* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.2.0"></a>
### [v1.2.0](/compare/v1.1.0...v1.2.0)

> 2026-05-05

#### 🩹 Fixes

* **changelog:** make tag prefix configurable
* **cmd:** wire configurable tag prefix
* **git:** stop hardcoding tag prefix

#### 📖 Documentation

* **config:** document tag prefix setting


<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.1.0

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.1.0](#v1.1.0)
* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.1.0"></a>
### [v1.1.0](/compare/v1.0.1...v1.1.0)

> 2026-05-01

#### 🚀 Enhancements

* **config:** add template override sample


<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v1.0.1

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.0.1](#v1.0.1)
* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.0.1"></a>
### [v1.0.1](/compare/v1.0.0...v1.0.1)

> 2026-05-01

#### 💅 Refactors

* update module path and imports to use nicolaiort namespace


<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 🏡 Chore

* **release:** prepare v1.0.0

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v1.0.0](#v1.0.0)
* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v1.0.0"></a>
### [v1.0.0](/compare/v0.6.1...v1.0.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** pass release tag to hooks

#### 📖 Documentation

* **readme:** add go install instructions


<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag

#### 🏡 Chore

* **release:** prepare v0.6.1


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v0.6.1](#v0.6.1)
* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.6.1"></a>
### [v0.6.1](/compare/v0.6.0...v0.6.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** fetch full history for release notes
* **cli:** push release commit with tag


<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.6.0

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v0.6.0](#v0.6.0)
* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.6.0"></a>
### [v0.6.0](/compare/v0.5.1...v0.6.0)

> 2026-05-01

#### 🚀 Enhancements

* **ci:** use changelog command for release notes
* **cli:** add release hooks
* **cli:** add repo config file support


<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.5.1

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v0.5.1](#v0.5.1)
* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.5.1"></a>
### [v0.5.1](/compare/v0.5.0...v0.5.1)

> 2026-05-01

#### 📖 Documentation

* **changelog:** restore type headings


<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🩹 Fixes

* **cli:** trim changelog output

#### 🏡 Chore

* **release:** prepare v0.5.0

#### 🚀 Enhancements

* **cli:** add changelog subcommand


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01

#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1

#### 🚀 Enhancements

* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.4.0

#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01

#### 🩹 Fixes

* **changelog:** normalize release tag names

#### 🏡 Chore

* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01

#### 🩹 Fixes

* **ci:** correct taskfile command syntax

#### 🏡 Chore

* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0

#### 🚀 Enhancements

* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.2.0

#### 📖 Documentation

* **copilot:** document e2e smoke tests

#### ✅ Tests

* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01

#### 🩹 Fixes

* **cli:** skip push prompt for flag releases

#### 🏡 Chore

* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01

#### 🏡 Chore

* **release:** prepare v0.1.3

#### 💅 Refactors

* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01

#### 🩹 Fixes

* **cli:** stage default changelog path

#### 🏡 Chore

* **release:** prepare v0.1.2

#### 💅 Refactors

* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01

#### 🩹 Fixes

* **cli:** commit release artifacts

#### 🏡 Chore

* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01

#### 🩹 Fixes

* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests

#### 📖 Documentation

* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions

#### 🚀 Enhancements

* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling

#### 💅 Refactors

* **cli:** make release the root command



# Changelog

## Version History

* [v0.5.0](#v0.5.0)
* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.5.0"></a>
### [v0.5.0](/compare/v0.4.2...v0.5.0)

> 2026-05-01

#### 🚀 Enhancements

* **cli:** add changelog subcommand

#### 🩹 Fixes

* **cli:** trim changelog output


<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01
#### 🩹 Fixes

* **ci:** stage release assets before upload

#### 🏡 Chore

* **ci:** bump github actions versions
* **release:** prepare v0.4.2


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01
#### 🚀 Enhancements

* **scripts:** add release installer

#### 🩹 Fixes

* **ci:** checkout repo in release job
* **scripts:** use bash installer

#### 🏡 Chore

* **release:** prepare v0.4.1


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01
#### 📖 Documentation

* add motivation section to README
* add initial README with installation and usage instructions

#### 🏡 Chore

* **release:** prepare v0.4.0


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01



* **changelog:** normalize release tag names



* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01



* **ci:** correct taskfile command syntax



* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0



* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01



* **release:** prepare v0.2.0



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [v0.4.2](#v0.4.2)
* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.4.2"></a>
### [v0.4.2](/compare/v0.4.1...v0.4.2)

> 2026-05-01



* **ci:** stage release assets before upload



* **ci:** bump github actions versions


<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01



* **ci:** checkout repo in release job
* **scripts:** use bash installer



* **release:** prepare v0.4.1



* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01



* **release:** prepare v0.4.0



* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01



* **changelog:** normalize release tag names



* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01



* **ci:** correct taskfile command syntax



* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0



* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01



* **release:** prepare v0.2.0



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [v0.4.1](#v0.4.1)
* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.4.1"></a>
### [v0.4.1](/compare/v0.4.0...v0.4.1)

> 2026-05-01



* **ci:** checkout repo in release job
* **scripts:** use bash installer



* **scripts:** add release installer


<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01



* **release:** prepare v0.4.0



* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01



* **changelog:** normalize release tag names



* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01



* **ci:** correct taskfile command syntax



* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0



* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01



* **release:** prepare v0.2.0



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [v0.4.0](#v0.4.0)
* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.4.0"></a>
### [v0.4.0](/compare/v0.3.1...v0.4.0)

> 2026-05-01



* add motivation section to README
* add initial README with installation and usage instructions


<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01



* **changelog:** normalize release tag names



* **release:** prepare v0.3.1


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01



* **ci:** correct taskfile command syntax



* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0



* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01



* **release:** prepare v0.2.0



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [v0.3.1](#v0.3.1)
* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.3.1"></a>
### [v0.3.1](/compare/v0.3.0...v0.3.1)

> 2026-05-01



* **changelog:** normalize release tag names


<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01



* **ci:** correct taskfile command syntax



* **changelog:** remove outdated changelog template
* **release:** prepare v0.3.0



* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01



* **release:** prepare v0.2.0



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [v0.3.0](#v0.3.0)
* [v0.2.0](#v0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="v0.3.0"></a>
### [v0.3.0](/compare/v0.2.0...v0.3.0)

> 2026-05-01



* **ci:** correct taskfile command syntax



* **changelog:** remove outdated changelog template



* **ci:** add tagged release workflow and build task


<a name="v0.2.0"></a>
### [v0.2.0](/compare/v0.1.5...v0.2.0)

> 2026-05-01



* **release:** prepare v0.2.0



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [0.2.0](#0.2.0)
* [v0.1.5](#v0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="0.2.0"></a>
### [0.2.0](/compare/v0.1.5...0.2.0)

> 2026-05-01



* **copilot:** document e2e smoke tests



* **e2e:** add release smoke coverage


<a name="v0.1.5"></a>
### [v0.1.5](/compare/v0.1.4...v0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases



* **release:** prepare v0.1.5


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [0.1.5](#0.1.5)
* [v0.1.4](#v0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="0.1.5"></a>
### [0.1.5](/compare/v0.1.4...0.1.5)

> 2026-05-01



* **cli:** skip push prompt for flag releases


<a name="v0.1.4"></a>
### [v0.1.4](/compare/v0.1.3...v0.1.4)

> 2026-05-01



* **release:** prepare v0.1.4


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [0.1.4](#0.1.4)
* [v0.1.3](#v0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="0.1.4"></a>
### [0.1.4](/compare/v0.1.3...0.1.4)

> 0001-01-01


<a name="v0.1.3"></a>
### [v0.1.3](/compare/v0.1.2...v0.1.3)

> 2026-05-01



* **release:** prepare v0.1.3



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



# Changelog

## Version History

* [0.1.3](#0.1.3)
* [v0.1.2](#v0.1.2)
* [v0.1.1](#v0.1.1)
* [v0.1.0](#v0.1.0)

## Changes

<a name="0.1.3"></a>
### [0.1.3](/compare/v0.1.2...0.1.3)

> 2026-05-01



* **changelog:** restructure release changelog template for improved clarity


<a name="v0.1.2"></a>
### [v0.1.2](/compare/v0.1.1...v0.1.2)

> 2026-05-01



* **cli:** stage default changelog path



* **release:** prepare v0.1.2



* **cli:** update command usage from 'release' to 'shikai'


<a name="v0.1.1"></a>
### [v0.1.1](/compare/v0.1.0...v0.1.1)

> 2026-05-01



* **cli:** commit release artifacts



* **changelog:** add initial changelog file with version 0.1.0 details
* **release:** prepare v0.1.1


<a name="v0.1.0"></a>
### v0.1.0

> 2026-05-01



* **changelog:** use git-chglog library
* **manifest:** skip unsupported manifest types
* **release:** correct bump logic and add tests



* add copilot-instructions.md
* **copilot:** update commit guidance
* **copilot:** update changelog instructions
* **copilot:** enhance commit guidelines for logical grouping and clarity
* **copilot:** add repository instructions



* add changelog template and configuration files
* initial release CLI implementation
* **interactive:** add arrow selector
* **release:** normalize release references and add tests for version handling



* **cli:** make release the root command



## [0.1.2] - 2026-05-01
### Bug Fixes
- stage default changelog path
### Refactoring
- update command usage from 'release' to 'shikai'


## [0.1.1] - 2026-05-01
### Bug Fixes
- commit release artifacts
### Chores
- add initial changelog file with version 0.1.0 details


## [0.1.1] - 2026-05-01
### Bug Fixes
- commit release artifacts
### Chores
- add initial changelog file with version 0.1.0 details


## [0.1.0] - 2026-05-01
### Bug Fixes
- use git-chglog library
- skip unsupported manifest types
- correct bump logic and add tests
### Documentation
- add copilot-instructions.md
- update commit guidance
- update changelog instructions
- enhance commit guidelines for logical grouping and clarity
- add repository instructions
### Features
- add changelog template and configuration files
- initial release CLI implementation
- add arrow selector
- normalize release references and add tests for version handling
### Refactoring
- make release the root command
